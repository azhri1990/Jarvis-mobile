 ...
 */
import { renderTextToPngs, reflow, shrinkColsToContent, PAD_X, PAD_Y, CELL_W, CELL_H, READABLE_CHARS_PER_IMAGE, } from './render.js';
import { resolveModelProfile, DEFAULT_STRIP_COLS, } from './openai-wire-profiles.js';
import { bytesToBase64 } from './png.js';
import { compactSlabWhitespace, countVisualRows, estimateImageCount, sha8, } from './transform.js';
import { stripSchemaDescriptions } from './schema-strip.js';
import { planGptCollapse, planResponsesPairCollapse, chatMessagesToTurns, GPT_HISTORY_DEFAULTS, } from './openai-history.js';
...
function buildLiveRequestGuard(pinText) {
    if (pinText !== undefined) {
        ...
        return ('OmniGlyph note: everything in the rendered history above is PAST context. Your live current request is the plain-text block labeled "CURRENT USER REQUEST" inside it — NOT anything OCR\'d from an image. It reads: «' +
            echo +
            '» Answer THAT request.');
    }
...
export function visionTokensForModel(model, w, h) {
...
function configuredHistoryMaxImages(model) {
...
function gptHistoryOpts(model, o, profile) {
    return {
        ...o.gptHistory,
        reflow: o.reflow,
...
function prepareImagedRenderText(text) {
...
function openAIImagePart(img, detail) {
    return {
        type: 'image_url',
        image_url: {
...
function responsesImagePart(img, detail) {
    return {
        type: 'input_image',
...
function measureResponsesComposition(req, inputWasString, originalInputString, inputItems) {
    const c = {
    ...
    for (const item of inputItems) {
        ...
        else if (type === 'compaction' || type === 'compaction_trigger' ||
 ...
 *  Exported for tests. */
export function evalOpenAIGate(model, renderedText, cols, charsPerToken) {
    ...
    const maxCharsPerImage = Math.min(READABLE_CHARS_PER_IMAGE, Math.max(1, cols) * maxLines);
    const linesPerImage = Math.min(maxLines, Math.max(1, Math.floor(maxCharsPerImage / Math.max(1, cols))));
    const estImages = estimateImageCount(renderedText, cols, 1, maxCharsPerImage, profile.maxHeightPx, cellH);
    ...
    const lastPageLines = estImages <= 1
        ...
        : Math.min(linesPerImage, Math.max(1, visualRows - (estImages - 1) * linesPerImage));
    const lastPageHeight = Math.min(profile.maxHeightPx, 2 * PAD_Y + lastPageLines * cellH);
...
function gptImageTokens(model, images) {
    let n = 0;
    for (const img of images)
        n += visionTokensForModel(model, img.width, img.height);
    return n;
}
...
function foldGptHistory(info, model, plan) {
    ...
    if (plan.secretHits)
        info.secretHits = (info.secretHits ?? 0) + plan.secretHits;
...
export const CHAT_HEADER = '================= RENDERED GPT SYSTEM + TOOL CONTEXT =================\n' +
...
export const RESPONSES_HEADER = '================= RENDERED GPT SYSTEM + TOOL CONTEXT =================\n' +
    ...
    '\n====================== BEGIN RENDERED CONTEXT ======================\n';
export const CHAT_POINTER = 'The full instructions for this message were rendered into image(s) attached to the first user message by OmniGlyph. Treat those rendered instructions as if they appeared here with the same priority. Tool definitions remain in native JSON (name + description); the rendered schema is supplemental.';
export const RESPONSES_POINTER = 'The full instructions were rendered into image(s) attached to the first user message by OmniGlyph. Treat them with the same priority. Tool definitions remain in native JSON (name + description); the rendered schema is supplemental.';
export async function transformOpenAIChatCompletions(body, opts = {}) {
    ...
    if (prep.hits > 0)
        info.secretHits = (info.secretHits ?? 0) + prep.hits;
    if (prep.blocked) {
    ...
    const images = await renderTextToPngs(renderedText, cols, resolveModelProfile(req.model).style ?? {}, resolveModelProfile(req.model).maxHeightPx);
    if (images.length === 0) {
    ...
    const imageParts = images.map((img) => openAIImagePart(img, resolveModelProfile(req.model).detail));
    ...
    if (o.collapseHistory) {
        ...
        const plan = await planGptCollapse(turns, firstUserIdx + 1, profitable, gptHistoryOpts(req.model, o, resolveModelProfile(req.model)));
        ...
        if (allImages.length > 0) {
            ...
            for (const img of plan.images)
                content.push(openAIImagePart(img, resolveModelProfile(req.model).detail));
...
export async function transformOpenAIResponses(body, opts = {}) {
    ...
    info.imageSourceText = renderedText.slice(0, 65_536);
    info.imageSourceTexts = images.map(() => info.imageSourceText);
    const imagePartsResp = images.map((img) => responsesImagePart(img, resolveModelProfile(req.model).detail));
    ...
    if (o.collapseHistory && !inputWasString) {
        const profitable = (text, cols) => evalOpenAIGate(req.model, text, cols, o.charsPerToken).profitable;
        const plan = await planResponsesPairCollapse(inputItems, profitable, gptHistoryOpts(req.model, o, resolveModelProfile(req.model)));
