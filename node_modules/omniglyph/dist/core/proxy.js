...
import { isAnthropicMessagesPath, isModelImageable, isOmniGlyphSupportedGptModel, isOmniGlyphSupportedModel, } from './applicability.js';
...
function isCanonicalOpenAIPath(pathname, headers, hasOpenAIKey) {
    ...
    const looksAnthropicClient = bearerIsAnthropic || headers.has('anthropic-version') || headers.has('anthropic-beta');
    const looksOpenAIAuth = !looksAnthropicClient && (hasOpenAIKey || (headers.has('authorization') && !headers.has('x-api-key')));
...
async function sniffRefusal(res) {
    const contentType = res.headers.get('content-type') ?? '';
    if (!res.ok || res.body === null)
        return { verdict: 'ok', replay: res };
    const replayOf = (body) => new Response(body, { status: res.status, statusText: res.statusText, headers: res.headers });
    if (contentType.includes('text/event-stream')) {
        ...
        while (verdict === undefined) {
            ...
            if (/"type"\s*:\s*"content_block_delta"/.test(seen))
                verdict = 'ok';
            else if (buffered > REFUSAL_SNIFF_BUFFER_CAP)
                verdict = 'ok'; // fail-open
        }
    ...
    if (contentType.includes('application/json')) {
        ...
        try {
            ...
            if (json.stop_reason === 'refusal' && !hasText)
                verdict = 'refusal';
        }
        catch {
            // non-JSON body despite the content-type — fail-open
        }
        return { verdict, replay: replayOf(buf) };
    }
    return { verdict: 'ok', replay: res };
}
...
export function resolveUpstreams(config) {
    if (config.provider === 'cloudflare-ai-gateway') {
        ...
        if (!base) {
            throw new Error("provider 'cloudflare-ai-gateway' requires gatewayBaseUrl (OMNIGLYPH_GATEWAY_BASE_URL)");
        }
...
function sessionKeyOf(bodyIn) {
    try {
        const obj = JSON.parse(new TextDecoder().decode(bodyIn));
...
function estimateWarmPrefixTokens(bodyIn) {
        ...
        if (isMessages || isOpenAIChat || isOpenAIResponses) {
            ...
            try {
                ...
                if (isMessages && messagesAnthropic
                    ...
                    if (warm > 0)
                        effectiveOpts = { ...effectiveOpts, priorWarmTokens: warm };
                }
                const r = isMessages
                ...
                if (r.info.compressed)
                    markImaged(sessionKey);
                if (!modelOk)
                    r.info.reason = 'unsupported_model';
                else if (!imageOk)
                    r.info.reason = 'model_unverified';
