...
export async function planGptCollapse(turns, protectedPrefix, isProfitable, opts = {}) {
    ...
    for (const [s, e] of sections) {
        ...
        if (sectionGuard.hits > 0)
            secretHits += sectionGuard.hits;
...
function responseCallText(item) {
...
function classifyResponsesPairs(items, keepRecentPairs) {
    const calls = new Map();
    ...
    for (const id of ids) {
        ...
        if (cs.length === 1 && os.length === 1 && os[0] === cs[0] + 1) {
            ...
            completed.push({
                ...
                outputTokens: gptCountTokens(typeof output.output === 'string' ? output.output : safeJson(output.output)),
            });
            continue;
        }
        if (cs.length > 0 && os.length === 0)
            openCalls += cs.length;
        else if (os.length > 0 && cs.length === 0)
            orphanOutputs += os.length;
        else
            malformedItems += cs.length + os.length;
    }
...
function emptyResponsesPairPlan(state) {
    return {
...
export async function planResponsesPairCollapse(items, isProfitable, opts = {}) {
    const o = { ...GPT_HISTORY_DEFAULTS, ...opts };
    ...
    for (const pair of old) {
        const run = runs.at(-1);
        if (run && pair.callIndex === run.at(-1).outputIndex + 1)
            run.push(pair);
        else
            runs.push([pair]);
    }
    const renderPairs = async (pairs) => {
        ...
        if (o.idsBlock)
            renderedText = appendIdsBlock(renderedText);
    ...
    for (const run of runs) {
        ...
        if (!isProfitable(best.renderedText, o.cols))
            continue;
    ...
    const selectedPairs = old.filter((pair) => selectedIds.has(pair.callIndex) && selectedIds.has(pair.outputIndex));
