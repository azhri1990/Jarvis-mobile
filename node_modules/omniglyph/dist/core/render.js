...
export const DENSE_RENDER_STYLE = { cellWBonus: 0, cellHBonus: 0, aa: false };
...
function isEscapeExempt(cp) {
    if (cp < 0x20)
        return true; // C0 controls (slot markers; \t is expanded before we run)
    if (cp >= 0x7f && cp <= 0x9f)
    ...
    if (cp >= 0xfe00 && cp <= 0xfe0f)
        return true; // variation selectors (emoji presentation)
...
export function escapeMissingGlyphs(line) {
    let out = null; // lazily materialized on first miss
    let i = 0;
