...
function lineSignalsMachineState(line) {
...
function hasExactMachineToken(text) {
 ...
 */
export function shouldKeepToolResultSharp(block) {
    if (!block.text)
        return false;
    const sample = boundedSample(block.text);
    if (sample.includes('```') || looksStructured(sample))
        return true;
    for (const line of sample.split('\n')) {
        if (lineSignalsMachineState(line))
            return true;
    }
...
export function resolveCompressionProfile(raw) {
    const value = (raw ?? '').trim().toLowerCase();
    if (!value || value === 'aggressive' || value === 'legacy')
    ...
    throw new Error(`invalid OMNIGLYPH_PROFILE '${raw}'; expected coding-safe, balanced, aggressive or passthrough`);
}
function maxDefined(base, caller) {
    if (base === undefined)
        return caller;
    if (caller === undefined)
        return base;
...
function tightenAnthropicHistory(base, caller) {
    if (!base && !caller)
        return undefined;
    return {
        ...base,
...
function tightenGptHistory(base, caller) {
    if (!base && !caller)
        return undefined;
    return {
        ...base,
...
export function mergeCompressionProfileOptions(profile, overrides = {}) {
    ...
    const keepSharp = baseKeep && callerKeep
        ? (block) => {
            ...
            let caller = false;
            try {
                base = baseKeep(block) === true;
            }
            catch { /* retain caller result */ }
            try {
                caller = callerKeep(block) === true;
            }
            catch { /* retain base result */ }
            return base || caller;
        }
        : callerKeep ?? baseKeep;
    const merged = {
    ...
    return {
        ...
        compressReminders: false,
        compressToolResults: false,
        minCompressChars: maxDefined(profile.transform.minCompressChars, overrides.minCompressChars),
        collapseHistory: overrides.collapseHistory === false ? false : true,
        historyAmortizationHorizon: maxDefined(profile.transform.historyAmortizationHorizon, overrides.historyAmortizationHorizon),
        anthropicHistory: tightenAnthropicHistory(profile.transform.anthropicHistory, overrides.anthropicHistory),
