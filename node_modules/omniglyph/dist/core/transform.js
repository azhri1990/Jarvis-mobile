 ...
 */
import { renderTextToPngsMultiCol, reflow, maxFittingCols, shrinkColsToContent, MAX_HEIGHT_PX, NL_SENTINEL, neutralizeSentinel, PAD_X, PAD_Y, CELL_W, CELL_H, READABLE_CHARS_PER_IMAGE, DENSE_CONTENT_CHARS_PER_IMAGE, DENSE_RENDER_STYLE, } from './render.js';
...
import { ANTHROPIC_IMAGE_BLOCK_OVERHEAD_TOKENS, anthropicImageTokens, pageGeometryForTier, resolveAnthropicVisionTier, STANDARD_PAGE, } from './anthropic-vision.js';
const DEFAULTS = {
    compress: true,
    compressTools: true,
 ...
 */
export const CLAUDE_CODE_OAUTH_IDENTITY = "You are Claude Code, Anthropic's official CLI for Claude.";
...
function imageTokensForRows(visualRows, cols, numCols = 1, imageCountCap, maxCharsPerImage = READABLE_CHARS_PER_IMAGE, tier = 'standard') {
...
function imageTokensCost(text, cols, numCols = 1, imageCountCap, shrinkWidth = true, maxCharsPerImage = READABLE_CHARS_PER_IMAGE, tier = 'standard') {
...
function denseGateGeometry(cols, numCols, page = STANDARD_PAGE) {
...
export function evalCompressionProfitability(text, cols, imageCountCap = undefined, numCols = 1, charsPerToken = CHARS_PER_TOKEN, priorWarmTokens = 0, priorWarmImageTokens = 0, shrinkWidth = true, maxCharsPerImage = READABLE_CHARS_PER_IMAGE, tier = 'standard') {
    const n = Math.max(1, numCols | 0);
...
export function isCompressionProfitable(text, cols = DEFAULTS.cols, imageCountCap, numCols = 1, charsPerToken = CHARS_PER_TOKEN, priorWarmTokens = 0, priorWarmImageTokens = 0, shrinkWidth = true, maxCharsPerImage = READABLE_CHARS_PER_IMAGE, tier = 'standard') {
    const n = Math.max(1, numCols | 0);
    if (typeof text !== 'string' || text.length === 0)
        return false;
    const cpt = Number.isFinite(charsPerToken) && charsPerToken > 0
 ...
 */
export function isCompressionProfitableAmortized(text, cols, imageCountCap, numCols, charsPerToken, horizon, priorWarmTokens = 0, priorWarmImageTokens = 0, shrinkWidth = true, maxCharsPerImage = READABLE_CHARS_PER_IMAGE, tier = 'standard') {
...
function extractSystemText(sys) {
    ...
    const hasCacheControlledText = sys.some((block) => block && block.type === 'text' && block.cache_control !== undefined);
    const textParts = [];
    const kept = [];
...
function isGlobalScopeCacheControl(cc) {
    return typeof cc === 'object' && cc !== null && cc.scope === 'global';
}
/**
...
function isToolSearchManaged(t) {
    const rec = t;
    if (rec['defer_loading'] === true)
        return true;
    const type = rec['type'];
...
export function countVisualRows(text, cols) {
    let rows = 0;
    let lineStart = 0;
...
export function estimateImageCount(textOrLen, cols, numCols = 1, maxCharsPerImage = READABLE_CHARS_PER_IMAGE, maxHeightPx = MAX_HEIGHT_PX, cellH = CELL_H) {
    const n = Math.max(1, numCols | 0);
 ...
 *  for tests. */
export function truncateForBudget(text, maxImages, cols, numCols = 1, maxCharsPerImage = DENSE_CONTENT_CHARS_PER_IMAGE, maxHeightPx = MAX_HEIGHT_PX) {
    const n = Math.max(1, numCols | 0);
    ...
    const packedRows = (chars) => Math.ceil(chars / Math.max(1, cols));
    const rowCost = (segment, priorChars, addedChars) => reflowed
...
page = STANDARD_PAGE) {
    ...
    if (blockGuard.blocked)
        return null; // caller keeps the block as native text
...
async function runHistoryCollapseAndFinalize(req, info, o, opts, droppedCodepoints, tier = 'standard', page = STANDARD_PAGE) {
    ...
    if (Array.isArray(req.messages) && req.messages.length > 0) {
        ...
        const historyProfitable = (text, cols) => {
            ...
            return isCompressionProfitableAmortized(text, g.cols, undefined, 1, historyCpt, horizon, o.priorWarmTokens, o.priorWarmImageTokens, true, g.maxChars, tier);
        };
        ...
        const { messages: newMessages, info: histInfo } = await collapseHistory(req.messages, historyProfitable, {
            ...o.anthropicHistory,
        ...
        if (histInfo.secretHits)
            info.secretHits = (info.secretHits ?? 0) + histInfo.secretHits;
...
export async function transformRequest(body, opts = {}) {
    ...
    if (o.compressTools && !keepSystemText && Array.isArray(req.tools) && req.tools.length > 0) {
        ...
        if (deferredSkipped > 0)
            info.deferredToolsSkipped = deferredSkipped;
    }
    ...
    if (!keepSystemText) { // keepSystemText: slab stays text (see the note above section 2)
        ...
        if (slabGuard.hits > 0)
            info.secretHits = (info.secretHits ?? 0) + slabGuard.hits;
        ...
        const slabGateEval = evalCompressionProfitability(combinedWithHeader, slabCols, undefined, numCols, slabCpt, o.priorWarmTokens, o.priorWarmImageTokens, false, // already shrunk — don't double-shrink
        ...
        for (let i = 0; i < images.length; i++) {
            ...
            const markThisPage = systemStaticCacheControl !== undefined
                && (i === images.length - 1 || isGlobalScopeCacheControl(systemStaticCacheControl));
            imageBlocks.push(markThisPage
    ...
    const volatileEnvText = hasUserMsg && !keepSystemText ? volatileEnvParts.join('\n\n') : '';
            ...
            if (firstUserIdx >= 0) {
                ...
                if (o.compressReminders) {
                    for (const blk of existing) {
                        ...
                        if (reminderGuard.hits > 0)
                            info.secretHits = (info.secretHits ?? 0) + reminderGuard.hits;
                        ...
                        const { blocks: imgs, pngs: rawPngs, dims: rawDims, droppedChars, droppedCodepoints: dcp, pixels } = reminderImaged;
                    ...
                    if (blk && blk.type === 'tool_result') {
                        ...
                        if (typeof innerRaw === 'string') {
                            ...
                            if (innerR.length < o.minToolResultChars) {
                                ...
                                if (trGuard.hits > 0)
                                    info.secretHits = (info.secretHits ?? 0) + trGuard.hits;
                                ...
                                if (trImaged === null) {
                                    ...
                                    const { blocks: imgs, pngs: rawPngs, dims: rawDims, droppedChars, droppedCodepoints: dcp, pixels } = trImaged;
                                    (info.imagePngs ??= []).push(...rawPngs);
                                    (info.imageDims ??= []).push(...rawDims);
                                    for (const img of imgs)
                                        info.imageBytes += approxBlockBytes(img);
                            ...
                            for (const ib of innerRaw) {
                                ...
                                if (partGuard.hits > 0)
                                    info.secretHits = (info.secretHits ?? 0) + partGuard.hits;
                                ...
                                const { blocks: imgs, pngs: rawPngs, dims: rawDims, droppedChars, droppedCodepoints: dcp, pixels } = partImaged;
                                (info.imagePngs ??= []).push(...rawPngs);
                                (info.imageDims ??= []).push(...rawDims);
