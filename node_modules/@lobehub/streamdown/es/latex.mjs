...
function escapeCurrencyDollars(text) {
	...
	let content = text.replaceAll(/(```[\s\S]*?```|`[^\n`]*`|\$\$[\s\S]*?\$\$|(?<!\\)\$(?!\$)[^$]*\\[^$]*(?<!\\)\$(?!\$)|\$\d+\$|\$-?\d+(?:,-?\d+)+\$|\\\[[\s\S]*?\\\]|\\\(.*?\\\))/g, (match) => manager.add(match));
...
* ```
*/
function preprocessLaTeXStrict(text) {
...
* ```
*/
function preprocessLaTeXMinimal(text) {
	return preprocessLaTeX(text, {
		convertBrackets: true,
		escapeCurrency: true,
		escapeMhchem: false,
...
//#endregion
export { convertLatexDelimiters, escapeCurrencyDollars, escapeLatexPipes, escapeMhchemCommands, escapeTextUnderscores, fixCommonLaTeXErrors, handleCJKWithLatex, isLastFormulaRenderable, normalizeLatexSpacing, preprocessLaTeX, preprocessLaTeXMinimal, preprocessLaTeXStrict, validateLatexExpressions };
